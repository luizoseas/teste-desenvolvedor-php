...
<script>
    ...
    function deleteMass(){
        if(ListRemoveMass.length > 0){
            $.ajax({
                url: "{{route('api.usuarios.deleteMass')}}",
                type: "DELETE",
                data: {
    ...
    async function get(url = ""){
        var response
        if(url == ""){
            response = await fetch(URL_BASE+"/api/usuarios/get?per_page="+per_page+"&search="+search);
        }else{
        ...
        if(response.ok){
            ...
            if(json.status == "success"){
                ...
                data.data.forEach(function(cliente){
                    ListSelect.push(cliente.codUsuario);
                    $("#bodytable").append(`
                        <tr>
                            <th scope="row"><input type="checkbox" id="check${cliente.codUsuario}" onChange="ListMass(${cliente.codUsuario})" /></th>
                            <th scope="row">${cliente.codUsuario}</th>
