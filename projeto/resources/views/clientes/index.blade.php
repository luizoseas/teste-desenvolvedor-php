...
<script>
    ...
    function deleteMass(){
        if(ListRemoveMass.length > 0){
            $.ajax({
                url: "{{route('api.clientes.deleteMass')}}",
                type: "DELETE",
                data: {
    ...
    async function get(url = ""){
        var response
        if(url == ""){
            response = await fetch(URL_BASE+"/api/clientes/get?per_page="+per_page+"&search="+search);
        }else{
        ...
        if(response.ok){
            ...
            if(json.status == "success"){
                ...
                data.data.forEach(function(cliente){
                    ListSelect.push(cliente.codCliente);
                    $("#bodytable").append(`
                        <tr>
                            <th scope="row"><input type="checkbox" id="check${cliente.codCliente}" onChange="ListMass(${cliente.codCliente})" /></th>
